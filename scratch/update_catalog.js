...
const startMarker = 'const catalog = [';
const endMarker = '];';
...
if (startIndex !== -1 && endIndex !== -1) {
    const newContent = content.substring(0, startIndex + startMarker.length) + 
                       '\n  ' + newCatalog.map(item => JSON.stringify(item)).join(',\n  ') + 
                       '\n' + content.substring(endIndex);
    fs.writeFileSync(filePath, newContent, 'utf8');
    console.log("Catálogo atualizado com sucesso!");
} else {
    console.error("Não foi possível localizar o array catalog no arquivo site.js");
}
